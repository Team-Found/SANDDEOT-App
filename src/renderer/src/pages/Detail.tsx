...
export default function Detail(): JSX.Element {
  ...
  // resultRef.current?.appendChild(doc.out(its.markedUpText));
  ...
  useEffect(() => {
    doc.sentences().each((s: winkNLP.ItemSentence, k) => {
      ...
      if (a > 0.9) {
        ...
        s.markup("<mark class=\"highlight bg-opacity-80 hi-2\">", "</mark>");
      } else if (a > 0.4) {
        s.markup("<mark class=\"highlight bg-opacity-60 hi-3\">", "</mark>");
      } else if (a > 0.1) {
        s.markup("<mark class=\"highlight bg-opacity-40 hi-4\">", "</mark>");
      }
